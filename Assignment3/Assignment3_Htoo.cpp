#include <iostream>
#include <stdexcept>  // For std::out_of_range and std::invalid_argument exceptions
...
public:
    // Constructor: Initializes a matrix with specified number of rows and columns
    ...
    // Accessor method to get and set matrix elements with bounds checking
...
Matrix<T> scalarMultiply(const Matrix<T>& mat, T scalar) {
    ...
    for (int i = 0; i < mat.getRowCount(); ++i) {
        for (int j = 0; j < mat.getColumnCount(); ++j) {
            result.at(i, j) = mat.at(i, j) * scalar;  // Multiply each element by scalar
        }
    }
    return result;
}
